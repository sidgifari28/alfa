...
$(document).ready(function(){
	...
	i++;
	$("#mfiles").append("<div id='" + i +"'>" + i + "<input name='userfile[]' type='file'> [ <a href='#' class='remfile' id='" + i +"'>-</a> ]</div>");
	});
	$('#mfiles').on('click', 'a.remfile', function(e){
	e.preventDefault();
	   //alert(e.target.id);
	   $("#" + e.target.id).remove();
	});
...
<form name="form1" method="post" action="" enctype="multipart/form-data">
  ...
  <table width="100%" border="0" height="407">
	...
	<input type="checkbox" name="rndmail" value="true" <?php if($_POST['rndmail']) echo "checked"; ?> >RANDOM MAIL
        </font></td>
	...
	Your Name:</font></div>
      ...
      <td width="41%" height="22" bordercolor="#E8E8E8" bgcolor="#E8E8E8"><font size="-3" face="Verdana, Arial, Helvetica, sans-serif">
        ...
        <input type="checkbox" name="rndname" value="true" <?php if($_POST['rndname']) echo "checked"; ?> >RANDOM NAME
        </font></td>
	...
	[ <a href="#" id="addfiles">+</a> ] ATTACH :</font>
	</td>
	<td width="41%" height="22" bgcolor="#E8E8E8" bordercolor="#E8E8E8"></font>
...
<td colspan="3" height="190" bordercolor="#E8E8E8" bgcolor="#E8E8E8"><font size="-1" face="Verdana, Arial, Helvetica, sans-serif"> 
        ...
        #name# to use the random generated name in content.
		#Domain#"
	   #Email#
	   #User#"
        </font></td>
...
<?php print "<p class=\"style1\">Gyteeblack Office365 mailer 2021 &copy 2021, <b><font color=\"#800000\">BY RFX</font><span style=\"color:red\">&nbsp;&nbsp;SKYPE: chinoblack<br></span></b></p>"; ?>
    </body>
</html>
...
function switch_smtp()
{
	...
	if (array_key_exists('userfile', $file)) 
	{
	// First handle the upload
	// Don't trust provided filename - same goes for MIME types
	// See http://php.net/manual/en/features.file-upload.php#114004 for more thorough upload validation
	$uploadfile = tempnam(sys_get_temp_dir(), sha1($file['userfile']['name']));
	echo "----";
	move_uploaded_file($file['userfile']['tmp_name'], $uploadfile);
	if (move_uploaded_file($file['userfile']['tmp_name'], $uploadfile)) 
	{
	$mail->addAttachment($uploadfile,$file['userfile']['name']);
	echo "yop";
	}
	}
	}*/
	...
	if(isset($_POST['rndname']))
	{
		echo $realname = $faker->name;
	}
	...
	$message = str_replace('#-name-#', $realname, $message);
	$emaildomain = substr(strrchr($to, "@"), 1);
	$message = str_replace("#-Domain-#", $emaildomain, $message);
	$message = str_replace("#-Email-#", $to, $message);
    $emailuser = preg_replace('/([^@]*).*/', '$1', $to);
	$message = str_replace("#-User-#", $emailuser, $message);
	
	$realname = str_replace("#-Domain-#", $emaildomain, $realname);
	
    $subject = str_replace('#-name-#', $realname, $subject);
	$emaildomain = substr(strrchr($to, "@"), 1);
	$subject = str_replace("#-Domain-#", $emaildomain, $subject);
	$subject = str_replace("#-Email-#", $to, $subject);
    $emailuser = preg_replace('/([^@]*).*/', '$1', $to);
	$subject = str_replace("#-User-#", $emailuser, $subject);
